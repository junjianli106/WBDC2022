...
class WBDCCollator:
    ...
    def pad_and_truncate(self, input_ids_list, attention_mask_list,
                         visual_embedding_list, visual_attention_mask_list,
                         max_seq_len):
        ...
        return input_ids, attention_mask, \
                   visual_embedding, visual_attention_mask
    ...
    def __call__(self, examples: list) -> dict:
        ...
        input_ids, attention_mask, visual_embedding, visual_attention_mask = \
                self.pad_and_truncate(input_ids_list, attention_mask_list,
                                      visual_embedding_list, visual_attention_mask_list,
                                      max_seq_len)
...
def load_dataset(args, i):
    with open(f'../wbdc2022/process_data/pkl/pretrain_{i}.pkl', 'rb') as f:
...
def build_model_and_tokenizer(args):
    ...
    model = WCUniModel(args, task=['mlm']).to(args.device)
...
#         read_data(args, tokenizer)
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for step, batch in enumerate(train_iterator):
            ...
            input_ids, attention_mask = batch_cuda['text_input_ids'], batch_cuda['attention_mask']
            visual_embeds, visual_attention_mask = batch_cuda['visual_embedding'], batch_cuda['visual_attention_mask']
            
            if args.fp16:
                with autocast(): # itm_loss
                    loss, masked_lm_loss = model(visual_embeds, visual_attention_mask, input_ids, attention_mask)
                    loss = loss.mean()
