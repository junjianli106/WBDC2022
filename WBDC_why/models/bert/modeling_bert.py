...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        self, text_input_ids=None, token_type_ids=None, position_ids=None, past_key_values_length=0, 
        video_inputs_embeds=None
    ):
        text_inputs_embeds = self.word_embeddings(text_input_ids)
        
        input_shape = text_inputs_embeds.size()[:-1]
        ...
        text_inputs_embeds = text_inputs_embeds + token_type_embeddings
        if self.position_embedding_type == "absolute":
            position_embeddings = self.position_embeddings(position_ids)
            text_inputs_embeds += position_embeddings
            
        visual_token_type_ids = torch.ones(video_inputs_embeds.size()[:-1], dtype=torch.long, device=self.position_ids.device)
        visual_token_type_embeddings = self.visual_token_type_embeddings(visual_token_type_ids)
        visual_position_ids = torch.zeros(*video_inputs_embeds.size()[:-1], dtype=torch.long, device=video_inputs_embeds.device)
        visual_position_embeddings = self.visual_position_embeddings(visual_position_ids)
        video_inputs_embeds = video_inputs_embeds + visual_position_embeddings + visual_token_type_embeddings
        
        embeddings = torch.cat([text_inputs_embeds, video_inputs_embeds], 1)
...
class UniBertModel(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
    ...
    def forward(self, video_feature, video_mask, text_input_ids, text_mask, text_token_type_ids): 
        
        video_emb = self.video_fc(video_feature)
        
        embedding_output = self.embeddings(text_input_ids=text_input_ids, video_inputs_embeds=video_emb)
