...
class BertModel(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.video_proj = nn.Sequential(
                            nn.Linear(768, config.hidden_size, bias=False),
                            nn.Dropout(0.2),
    ...
    def forward(self, video_feature, video_mask, text_input_ids, text_mask, text_token_type_ids): 
        ...
        video_mask = video_mask[:, None, None, :]
        video_mask = (1.0 - video_mask) * -10000.0
        
        video_embedding = self.video_proj(video_feature)
        
        text_embedding = self.embeddings(input_ids=text_input_ids)
        text_embedding = self.encoder(mode='text', hidden_states=text_embedding, attention_mask=text_mask)['last_hidden_state']

        encoder_outputs = self.encoder(mode='fusion', hidden_states=text_embedding, attention_mask=text_mask,
                                       encoder_hidden_states=video_embedding, encoder_attention_mask=video_mask)['last_hidden_state']
